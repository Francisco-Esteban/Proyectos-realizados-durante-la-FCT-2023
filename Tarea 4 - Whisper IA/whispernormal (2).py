    ...
    if processor and model :
        ...
        if os.path.exists(camino_al_audio_nuevo) :
            ...
            if os.path.exists(direccion_chunks_sin_nombre):
                ...
                segmentos = sorted([segmento for segmento in direccion_chunks_completa if segmento.startswith(nombre_para_archivos)])
                ...
                with torch.no_grad():
                    ...
                    salida_de_generacion = processor.batch_decode(dialogo_detectado, skip_special_tokens=True)[0]

                    salida_completa = str("")
                    salida_completa += salida_de_generacion + " "

                    salida_completa_limpia = salida_completa.strip()
                    salida_completa_limpia_str = str(salida_completa_limpia)

                    nombre_archivo_generado = f"texto_de_{archivo_seleccionado}"


                    if salida_completa_limpia :
                        print("La salida del texto tiene la longitud de", len(salida_completa_limpia_str), "caracteres")
                        print(salida_completa_limpia_str)
                ...
                print(f"El camino a los segmentos de audio no existe, compruebalo ---> {direccion_chunks_completa}")
        else :
